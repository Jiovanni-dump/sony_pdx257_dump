...
function elbe_shimanto_asahi_yodo_nagara_init
{
    local ssr_str="$1"
    typeset -a ssr_array
    ssr_array=($ssr_str)
    ...
    do
       ...
       do
          if [ "$subsys_name" == ${ssr_array[$num]} ]; then
            if [ "spss" == ${ssr_array[$num]} ]; then
              if [ "SM8550" != "$soc_id" ] && [ "SM8450" != "$soc_id" ]; then
                echo "enabled" > /sys/class/remoteproc/remoteproc$i/recovery
              else
                echo "disabled" > /sys/class/remoteproc/remoteproc$i/recovery
              fi
...
case $soc_id in
     SM6375)
        ...
        zambezi_murray_init "$ssr_list"
    ;;
     SM8650|SM8550|SM8450)
        ssr_list="adsp cdsp slpi mss wlan spss"
        if [ "$build_variant" != "user" ]; then
            ssr_list="adsp slpi mss wlan venus"
        fi
        elbe_shimanto_asahi_yodo_nagara_init "$ssr_list"
    ;;
    SM8750)
        ssr_list="adsp cdsp mss wlan spss"
        if [ "$build_variant" != "user" ]; then
            ssr_list="adsp mss wlan venus"
        fi
        elbe_shimanto_asahi_yodo_nagara_init "$ssr_list"
    ;;
    SM6475)
    ...
    ;;
    SM6450)
        ssr_list="adsp cdsp slpi mss wlan wpss"
        if [ "$build_variant" != "user" ]; then
            ssr_list="adsp slpi mss wlan venus wpss"
        fi
        columbia_init "$ssr_list"
    ;;
    *)
